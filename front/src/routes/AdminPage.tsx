...
function AdminPage() {
  ...
  return (
    <main className="screen-page screen-page--admin">
      ...
      <h2>{answer}</h2>
      <button onClick={() => socket.emit('quiz_start')} disabled={quizStarted}>Quiz Start</button>
      <button onClick={() => socket.emit('quiz_stop')} disabled={!quizStarted}>Quiz Stop</button>
      <br/><br/>
      <button onClick={() => socket.emit('next_quiz')} disabled={!quizStarted}>Next Quiz</button>
    </main>
  )
}
